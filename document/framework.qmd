...
- **Bachelor or Higher Rate**: [Visit Data Source](https://data.census.gov/table/ACSDT1Y2024.C15003?q=county+level+eduacation+data)

- **Policy Support**:

**AI-Related Policy Support**: A binary categorical feature indicating whether a county is mentioned in government policies related to AI development. Counties mentioned in such policies will be assigned a value of 1; otherwise, they will be assigned a value of 0.

This is a pretty hard feature to construct, to make it easier, I will simplify it by assigning the value based on if a state is mentioned in the policies. If a state is mentioned in the policies, all counties in that state will be assigned a value of 1; otherwise, they will be assigned a value of 0.

This data is collected using AI agents. Basically, build use api from openai, then use the api and customized prompt to collect the data. In the whole process, i asked the agent to collect the data in a structured way, which shows a clear evidence path.

The project first used Bing api to search the policies related to AI development, then used the snippet from the search results to construct the policy support feature. then use llm api like chatgpt to extract structured data.

- **Current Data Centers held county**: [Visit Data Source](https://www.pewresearch.org/short-reads/2025/10/24/what-we-know-about-energy-use-at-us-data-centers-amid-the-ai-boom/#how-many-data-centers-are-in-the-u-s-and-where-are-they)

https://epoch.ai/data/data-centers#related-work

Data were collected from publicly accessible pages on datacentermap.com using web scraping techniques.
in accordance with the website’s robots.txt.
No restricted API endpoints or authentication-protected resources were accessed.
